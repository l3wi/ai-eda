...
function adaptCommunityComponent(
  ...
  return {
    ...
    symbol: {
      pins: component.symbol.pins.map((pin: EasyEDACommunityPin) => ({
        number: pin.number,
        name: pin.name,
        ...
        // Default values for community components (no style info available)
        hasDot: false,
        hasClock: false,
        pinLength: 100, // default EasyEDA units
      })),
      // Community components don't have parsed shapes yet - use empty arrays
      rectangles: [],
      circles: [],
      ellipses: [],
      arcs: [],
    ...
    footprint: {
      ...
      type: inferFootprintType(component.footprint.pads),
      pads: component.footprint.pads.map((pad: EasyEDACommunityPad) => ({
        shape: pad.shape,
        centerX: pad.x,
        centerY: pad.y,
        width: pad.width,
        height: pad.height,
        layerId: pad.layer === 'F.Cu' ? 1 : pad.layer === 'B.Cu' ? 2 : 11,
        net: '',
        number: pad.number,
        holeRadius: pad.holeRadius || 0,
        points: pad.points ? pad.points.map(p => `${p.x} ${p.y}`).join(' ') : '',
